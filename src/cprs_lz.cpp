...
/*Original file: grit/libgrit/cprs_lz.cpp*/
...
*/
/*
 * Original file says (but no longer in force in this file):
...
/*
   wwylele, Oct 2015
       Minimal modify to fit in MApRX.
       Take some code from grit/cldib/cldib_core.h
*/
...
uint lz77gba_decompress(RECORD *dst, const RECORD *src)
{
    ...
    DWORD header= read32le(src->data);
	if((header&255) != CPRS_LZ77_TAG)
		return 0;

    DWORD flags;
	int ii, jj, dstS= header>>8;
    BYTE *srcL= src->data+4, *dstD= (BYTE*)malloc(dstS);
